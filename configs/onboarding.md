...
## 反模式:同形重试(thread #18 判例,2026-09-20 用户批准)

成员会话(尤其 watcher/cron 唤起的 headless 会话)的发射纪律:

- **R1 失败分类 + 同形即停**:同一命令失败第 2 次后,禁止第 3 次发射同形命令。先分类失败:**「命令结构」类**换形态修复(`cd` → `git -C`、链式 → 拆步、隐式路径 → 绝对路径);**「策略/环境」类**(沙箱拒绝、权限、路径形态不匹配)任何重试都无效——改路径落点或走批准的升权。修复必须作用于命令结构,不是重述。
- **R3 模板硬约束(最高优先)**:任务模板**不得包含任何依赖 cwd/env 的命令**;路径一律用**目标执行器的原生绝对形态**(或并列两种形态并标注适用执行器)。消灭失败类比失败后修复更根本。
- **R2(仅背景,不是条款)**:文本写出的"下一步命令"对发射通道无约束力("声明≠发射");真正的保险是 R3 让失败类不存在。模板无法校验"发射与声明逐字符同源",把 R2 写成条款只会制造"已声明合规"的假象——这正是 #149 根因的同型错误。
